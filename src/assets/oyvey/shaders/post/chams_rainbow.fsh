...
vec3 rainbowColor() {
    ...
    return hslToRgb(hue, saturation, lightness) * outlineColor.rgb;
}
...
vec4 calculateInnerGlow() {
    ...
    vec3 filledRgb = shaderFill != 0 ? rainbowColor() : fillColor.rgb;
    if (innerGlowEnabled != 0 && innerGlowThickness != 0) {
        ...
        filledRgb = mix(filledRgb, shaderFill != 0 ? rainbowColor() : fillColor.rgb, clamp(innerGlow * (0.55 + fillColor.a * 0.75), 0.0, 1.0));
    }
...
void main() {
    vec4 centerTexel = texture(DiffuseSampler, texCoord);
    vec3 rgbColor = rainbowColor();
    vec4 color = vec4(rgbColor, centerTexel.a);
