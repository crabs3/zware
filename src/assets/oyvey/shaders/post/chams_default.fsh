...
vec4 calculateInnerGlow(vec4 color) {
    vec4 centerTexel = texture(DiffuseSampler, texCoord);
    vec3 filledRgb = mix(fillColor.rgb, fillColor.rgb, clamp(float(shaderFill), 0.0, 1.0));
    if (innerGlowEnabled != 0 && innerGlowThickness != 0) {
        ...
        filledRgb = mix(filledRgb, filledRgb, clamp(innerGlow * (0.55 + fillColor.a * 0.75), 0.0, 1.0));
    }
    ...
    for (int x = -lineWidth; x <= lineWidth; x++) {
        for (int y = -lineWidth; y <= lineWidth; y++) {
            ...
            if (sampledTexel.a > 0.0) {
                color = vec4(outlineColor.rgb, sampledTexel.a * outlineColor.a);
            }
        }
    }
    return color;
...
vec4 calculateOuterGlow(vec4 color) {
    if (glowThickness != 0 && color.a == 0.0) {
        ...
        color = vec4(outlineColor.rgb, glowIntensity * outlineColor.a * clamp(bestOpaqueColor.a, 0.0, 1.0));
    }
    return color;
}
...
void main() {
    ...
    if (color.a != 0.0) {
        color = calculateInnerGlow(vec4(color.rgb, 0.0));
    } else {
