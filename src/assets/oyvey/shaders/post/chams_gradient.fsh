...
vec3 gradientColor() {
    float gradientFactor = clamp(dot(texCoord, gradientAxis), 0.0, 1.0);
    return mix(gradientStartColor.rgb, gradientEndColor.rgb, gradientFactor) * outlineColor.rgb;
}

vec3 resolveInnerColor() {
    return shaderFill != 0 ? gradientColor() : fillColor.rgb;
}

vec4 calculateInnerGlow() {
    vec4 centerTexel = texture(DiffuseSampler, texCoord);
    vec3 filledRgb = resolveInnerColor();
    if (innerGlowEnabled != 0 && innerGlowThickness != 0) {
        ...
        filledRgb = mix(filledRgb, resolveInnerColor(), clamp(innerGlow * (0.55 + fillColor.a * 0.75), 0.0, 1.0));
    }
...
void main() {
    vec4 centerTexel = texture(DiffuseSampler, texCoord);
    vec4 color = vec4(gradientColor(), centerTexel.a);
